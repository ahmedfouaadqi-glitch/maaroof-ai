...
## الموجة الأولى — الأجزاء 9 و10 و11 (تعلّم الخبراء، حوكمة التعلّم، المعرفة الحيّة)

**لماذا تطوير لا إنشاء:** سجل القدرات في `src/lib/tool-catalog.ts` يصف *ما هي* الأداة، و`maaroof_memory` يخزّن ذاكرة الجلسات، و`platform_dna` يخزّن الأنماط مجهولة الهوية. لا شيء منها يعبّر عن *فهم معروف* لكل خبير ولا عن معرفة مهيكلة تتقادم وتُقاس. لذلك أُضيفت طبقة فوق الثلاثة، ولم يُستبدل أي منها.

### الجزء 9 — محرّك تعلّم الخبراء
- `src/lib/maaroof/experts.server.ts`: مقابلة إدراكية من 15 سؤالاً + حوار عكسي يسأل فيه الخبيرُ معروفاً، ثم استخراج JSON منظّم. طلبان للنموذج فقط لكل خبير.
- تقييم الفهم محلي بالكامل (`scoreUnderstanding`) عبر ست تغطيات: معرفة، قدرة، استدلال، ذاكرة، قرار، تعاون — بلا أي تكلفة نموذج إضافية.
- جداول: `expert_profiles` (الحالة الحالية)، `expert_learning_sessions` (النص الكامل + تقرير الفرق)، `expert_snapshots` (اللقطة المعتمدة التي يقرأها الوكيل).
- بصمة `fingerprint` لتعريف الأداة: إن لم يتغيّر التعريف تُعاد اللقطة **مجاناً** مع تسجيل سبب المجانية.

### الجزء 10 — حوكمة التعلّم
- `learning_budget_ledger` منفصل تماماً عن `token_ledger`: ولا سطر واحد من تكلفة التعلّم يمسّ رصيد أي مستخدم.
- كل عملية مسجّلة حتى المجانية، مع `zero_cost_reason`، وسقف شهري (`monthly_budget_usd`) يوقف التعلّم عند تجاوزه.
- تقرير الفرق بين كل نسخة والتي قبلها محفوظ في `expert_learning_sessions.diff`.

### الجزء 11 — المعرفة الحيّة
- `knowledge_nodes` + `knowledge_edges` بتسع طبقات (platform, expert, workspace, user, agent, mcp, tool, world, future).
- لكل عقدة: ثقة، موثوقية، أهمية، حداثة، عدد استخدام، ونسخة. الجودة محسوبة من مزيج هذه الإشارات؛ المعرفة تتقادم إن لم تُستخدم وتتقوّى إن نفعت.
- التقاط بعد كل جلسة واستدعاء قبل التخطيط — كلاهما كتلة نصية تُضاف لنفس الـ system prompt بلا طلب إضافي.

### التوافق الخلفي
- كل المفاتيح مُطفأة افتراضياً (`experts.enabled = false`, `knowledge.enabled = false`). عند الإطفاء يكون التوجيه مطابقاً حرفياً للسلوك السابق.
- لا صفحة إدارة جديدة: الأقسام الثلاثة (أكاديمية الخبراء، مرصد المعرفة، ميزانية التعلّم) أُضيفت داخل `MaaroofIntelligenceCenter` القائم.
- صلاحيات: كل الجداول الجديدة للمشرفين فقط، عدا عقد المعرفة المملوكة للمستخدم أو مساحة عمله.
