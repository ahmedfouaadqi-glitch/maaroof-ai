    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const project = asText(body.project_name, 200);
          ...
          const _chg = await chargeTokens({ userId, toolKey: "feasibility", runId: _runId, meta: { provider: "lovable_ai", model: _MODEL, endpoint: "/api/feasibility" } });
...
Return the JSON feasibility report now. All string fields MUST be in language "${lang}".`;
          ...
          const pack = await buildEvidencePack(`${project} ${body.sector || ""} ${body.city || ""} ${market.region}`.trim(), { limit: 4, lang });
          ...
          try {
            ...
            await _el({ runId: _runId, provider: "lovable_ai", model: _MODEL, endpoint: "/api/feasibility", inputTokens: Number(_u.prompt_tokens)||0, outputTokens: Number(_u.completion_tokens)||0, latencyMs: Date.now() - _t0 });
          } catch {}
