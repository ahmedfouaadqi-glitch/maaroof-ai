...
export function BrandPulseGauges() {
  ...
  const compute = async () => {
    ...
    // Activity (0..30): tools used in last 30 days
    ...
    // Brand boost trend (0..30): count + improvement signal
    const { data: runs } = await supabase
      .from("brand_boost_runs")
    ...
    // ── Gauge 2: AI Visibility Pulse — avg of latest analyses scores + freshness
    ...
    const lastDate = ans?.[0]?.created_at ? new Date(ans[0].created_at).getTime() : 0;
    const ageDays = lastDate ? (Date.now() - lastDate) / 864e5 : 999;
    const freshness = ageDays < 1 ? 20 : ageDays < 7 ? 14 : ageDays < 30 ? 8 : 0;
    const pulse = Math.min(100, Math.round(avg * 0.8 + freshness));
    ...
    const pulseHint =
      lang === "ar" ? `${scored.length} تحليل · آخر تحليل قبل ${Math.round(ageDays)} يوم`
  ...
  return (
    <div className="rounded-2xl border border-primary/20 bg-card/60 p-4 backdrop-blur">
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setTick((x) => x + 1)} className="ms-auto inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-primary">
          <RefreshCw className="size-3" /> {labels.refresh}
        </button>
      </div>
