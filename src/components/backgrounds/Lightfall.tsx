...
const Lightfall = ({
  ...
  // Stabilise the colors dependency so inline array literals don't rebuild the renderer.
  const colorsKey = (colors ?? []).join(",");
  const stableColors = useMemo(() => (colors ? [...colors] : undefined), [colorsKey]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // WebGL unavailable — render nothing, keep the page intact.
      return;
    }
    ...
    const { arr, count, avg } = prepColors(stableColors);
    ...
    const loop = (t: number) => {
      ...
      if (!paused && !hidden && programRef.current && meshRef.current) {
        try {
