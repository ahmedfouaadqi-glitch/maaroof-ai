...
export function CompetitorMonitor() {
  ...
  async function del(id: string) {
    if (!confirm("حذف المراقبة؟")) return;
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-card/40 p-4">
        ...
        <p className="mb-3 text-xs text-muted-foreground">يحفظ baseline ويسجّل تنبيهات عند تغيّر ظهور المنافسين (2 وحدة لكل إنشاء/فحص).</p>
        <div className="grid gap-2 sm:grid-cols-2">
          <input value={brand} onChange={(e) => setBrand(e.target.value)} placeholder="علامتك" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
          <input value={comps} onChange={(e) => setComps(e.target.value)} placeholder="منافسون مفصولون بفواصل" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </div>
        <div className="mt-2 flex items-center gap-2 text-xs">
          <label>كل (ساعات):</label>
      ...
      <div className="space-y-2">
        {items.length === 0 ? <p className="text-sm text-muted-foreground">لا توجد مراقبات بعد.</p> : items.map((w) => (
          <div key={w.id} className="rounded-lg border border-border bg-background/40 p-3 text-sm">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex gap-1">
                <button onClick={() => recheck(w.id)} className="rounded p-1.5 hover:bg-card" title="إعادة فحص"><RefreshCw className="size-4"/></button>
