...
export type AgentOpsSection = "tasks" | "channels";

/**
 * Operational surface of the autonomous agent, merged into the Maaroof page.
 * `section` decides which block renders; state stays inside a single instance.
 */
export function AgentOps({ section }: { section: AgentOpsSection }) {
  ...
  const [, setVisMsg] = useState<{ ok: boolean; text: string } | null>(null);
  ...
  useEffect(() => {
    if (!loading && !user) navigate({ to: "/auth", search: { mode: "signin", redirect: "/maaroof" } });
  ...
  const runNow = async (targetId?: string) => {
    ...
    try {
      ...
      if (res?.ok) setRunMsg({ ok: true, text: `${t("ag_run_done")} (${res.done || 0})` });
      else setRunMsg({ ok: false, text: `${t("ag_run_failed")} ${tx(res?.error)}` });
    } catch (e: any) {
  ...
  const sendCommand = async () => {
    ...
    try {
      ...
      if (res?.ok) { setCmdMsg({ ok: true, text: t("ag_cmd_ok") }); setCmd(""); }
      else setCmdMsg({ ok: false, text: `${t("ag_cmd_fail")} ${res?.error || ""}` });
    } catch (e: any) {
  ...
  const runVisibility = async () => {
    ...
    try {
      ...
      if (response.status === 401) setVisMsg({ ok: false, text: `${t("ag_vis_fail")} ${tx("auth_required")}` });
      else if (response.ok && res?.ok) setVisMsg({ ok: true, text: t("ag_vis_ok") });
      else setVisMsg({ ok: false, text: `${t("ag_vis_fail")} ${tx(res?.error)}` });
    } catch (e: any) {
      setVisMsg({ ok: false, text: `${t("ag_vis_fail")} ${e?.message || "unknown error"}` });
    } finally {
      setVisBusy(false);
      load();
    }
  ...
  if (loading || pageLoading) return (
    <div className="flex min-h-[40vh] items-center justify-center"><Loader2 className="size-8 animate-spin text-primary" /></div>
  );
  ...
  if (!sub) return (
    <div className="mx-auto max-w-2xl px-4 py-12 text-center">
      ...
      <h2 className="mt-6 font-display text-2xl font-bold text-gradient">{t("ag_no_sub_title")}</h2>
  ...
  if (section === "channels") return <ChannelsPanel onChanged={load} />;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-xl font-bold text-gradient">{addon?.name || t("ag_no_sub_title")}</h2>
            {isAdminSub && (
      ...
      {vis.isAgentFeatureVisible("command") && (
        <div className="rounded-2xl border border-accent/30 bg-gradient-to-br from-accent/5 to-primary/5 p-5">
          <h3 className="flex items-center gap-2 font-display text-lg font-bold text-gradient">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
            {cmdMsg && <span className={`text-xs ${cmdMsg.ok ? "text-success" : "text-destructive"}`}>{cmdMsg.text}</span>}
            <button
              onClick={sendCommand}
      ...
      <div className="rounded-2xl border border-border/60 bg-card/50 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {runMsg && <div className={`mt-2 text-xs ${runMsg.ok ? "text-success" : "text-destructive"}`}>{runMsg.text}</div>}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          ...
          {visible.map((tk) => (
            <div key={tk.id} className="rounded-xl border border-border bg-card/70 p-4">
              ...
              {tk.result?.summary && <p className="mt-2 whitespace-pre-wrap text-sm text-foreground/90">{tk.result.summary}</p>}
              {tk.result?.score != null && <div className="mt-2 text-xs">GEO Score: <b className="text-accent">{tk.result.score}/100</b></div>}
