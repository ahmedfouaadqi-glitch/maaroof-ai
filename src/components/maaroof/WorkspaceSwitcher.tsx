...
export function WorkspaceSwitcher({ onChange }: { onChange: (ws: Workspace | null) => void }) {
  ...
  async function remove(id: string) {
    if (!confirm("حذف مساحة العمل؟ الجلسات والذاكرة داخلها ستُفصل عنها.")) return;
    await del({ data: { id } });
    ...
    <div className="rounded-lg border bg-card p-3 space-y-2">
      ...
      {showForm && (
        <div className="space-y-2 p-2 rounded border bg-background/50">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="اسم العلامة/العميل" className="w-full border rounded px-2 py-1 bg-background text-sm" />
          <select value={kind} onChange={(e) => setKind(e.target.value as "own" | "client" | "brand")} className="w-full border rounded px-2 py-1 bg-background text-sm">
            ...
            <option value="own">خاصة بي</option>
          </select>
          <input value={brandUrl} onChange={(e) => setBrandUrl(e.target.value)} placeholder="رابط الموقع (اختياري)" className="w-full border rounded px-2 py-1 bg-background text-sm" />
      ...
      {loading ? (
        ...
        <p className="text-xs text-muted-foreground">لا مساحات بعد. أنشئ الأولى لعزل بيانات كل علامة/عميل.</p>
      ) : (
            ...
            <li key={w.id} className={`group flex items-center gap-2 p-2 rounded ${activeId === w.id ? "bg-primary/10 border border-primary/40" : "hover:bg-muted"}`}>
              <button onClick={() => pick(w)} className="flex-1 text-start truncate">
                ...
                <div className="text-[10px] text-muted-foreground">
                  {w.kind === "brand" ? "علامة" : w.kind === "client" ? "عميل" : "خاصة"}
                  {w.brand_url ? ` · ${new URL(w.brand_url).hostname.replace("www.", "")}` : ""}
                </div>
              </button>
              <button onClick={() => remove(w.id)} className="opacity-0 group-hover:opacity-100 text-destructive p-1 rounded hover:bg-destructive/10" title="حذف">
